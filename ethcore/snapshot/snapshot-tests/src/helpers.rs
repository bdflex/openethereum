...
pub fn restore(
	...
	let mut decompression_buffer = Vec::with_capacity(1024 * 1024);
	...
	for state_chunk_hash in manifest.state_hashes.iter() {
		...
		let len = snap::raw::Decoder::new().decompress(&chunk, &mut decompression_buffer).unwrap();
		state.feed(&decompression_buffer[..len], &flag)?;
	}
	...
	for chunk_hash in manifest.block_hashes.iter() {
		...
		let len = snap::raw::Decoder::new().decompress(&chunk, &mut decompression_buffer).unwrap();
		secondary.feed(&decompression_buffer[..len], engine, &flag)?;
	}
